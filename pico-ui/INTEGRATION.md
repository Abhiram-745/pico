...
character itself — but `bridge/server.mjs` still reads it as the art for the
drawn cursors, so it stays. Replace it with the project's own
`src/Pico.Desktop/Assets/pico.png` when wiring this up.
